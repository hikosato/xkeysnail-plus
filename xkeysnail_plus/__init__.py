...
def has_access_to_uinput():
    ...
    try:
        from xkeysnail_plus.output import _uinput  # noqa: F401
        return True
    except UInputError:
        return False
...
def cli_main():
    ...
    parser.add_argument('--devices', dest="devices", metavar='device', type=str, nargs='+',
                        help='keyboard devices to remap (if omitted, xkeysnail-plus choose proper keyboard devices)')
...
Make sure that you have executed xkeysnail-plus with root privilege such as

    $ sudo xkeysnail-plus config.py
""")
        import sys
        sys.exit(1)
    ...
    # Enter event loop
    from xkeysnail_plus.input import loop
