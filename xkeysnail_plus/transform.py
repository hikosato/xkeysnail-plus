...
def K(exp):
    ...
    while True:
        m = re.match(
            r"\A(LC|LCtrl|RC|RCtrl|C|Ctrl|LM|LAlt|RM|RAlt|M|Alt|LShift|RShift|Shift|LSuper|LWin|RSuper|RWin|Super|Win|Kata_Hira|Henkan|Muhenkan|Menu)-", exp)
        if m is None:
            break
        modifier = m.group(1)
...
def create_modifiers_from_strings(modifier_strs):
    ...
    for modifier_str in modifier_strs:
        ...
        elif modifier_str == 'Kata_Hira':
            modifiers.add(Modifier.KAHI)
        elif modifier_str == 'Henkan':
            modifiers.add(Modifier.HKN)
        elif modifier_str == 'Muhenkan':
            modifiers.add(Modifier.MHKN)
        elif modifier_str == 'Menu':
            modifiers.add(Modifier.MENU_)
    return modifiers
...
def transform_key(key, action, wm_class=None, quiet=False):
    ...
    if _mode_maps is None:
        ...
        if not quiet:
            print("WM_CLASS '{}' | active keymaps = [{}]".format(
                wm_class, ", ".join(keymap_names)))
