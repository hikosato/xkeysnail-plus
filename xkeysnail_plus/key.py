...
class Modifier(Enum):
    ...
    L_CONTROL, R_CONTROL, CONTROL, \
        L_ALT, R_ALT, ALT, \
        L_SHIFT, R_SHIFT, SHIFT, \
        L_SUPER, R_SUPER, SUPER, KAHI, \
        HKN, MHKN, MENU_ = range(16)
    ...
    def _get_modifier_map(cls):
        return {
            ...
            cls.KAHI: {Key.KATAKANAHIRAGANA},
            cls.HKN: {Key.HENKAN},
            cls.MHKN: {Key.MUHENKAN},
            cls.MENU_: {Key.COMPOSE}
        }
    ...
    def __str__(self):
        ...
        if self.value == self.KAHI.value: return "Kata_Hira"
        if self.value == self.HKN.value: return "Henkan"
        if self.value == self.MHKN.value: return "Muhenkan"
        if self.value == self.MENU_.value: return "Menu"
        return None
    ...
    def is_specified(self):
        return self.value == self.L_CONTROL.value or \
                ...
                self.value == self.KAHI.value or \
                self.value == self.HKN.value or \
                self.value == self.MHKN.value or \
                self.value == self.MENU_.value
